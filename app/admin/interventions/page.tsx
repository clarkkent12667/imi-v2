...
const getInterventions = cache(async (supabase: any) => {
...
const getLowPointsSummary = cache(async (supabase: any) => {
  const { data, error } = await supabase
...
export default async function AdminInterventionsPage() {
  ...
  const [interventions, lowPointsSummary] = await Promise.all([
    getInterventions(supabase),
    getLowPointsSummary(supabase),
  ])
    ...
    <div className="p-8">
      <div className="mb-8">
        ...
        <p className="text-muted-foreground">
          Track student low points and parent communication
        </p>
      </div>
